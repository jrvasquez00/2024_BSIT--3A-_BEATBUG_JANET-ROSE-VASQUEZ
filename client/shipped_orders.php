...
$sql_get_confirmed_orders = "SELECT i.product_name, i.product_price, i.product_desc, i.product_image, o.qty, o.order_date, o.order_id
                             FROM `orders` as o
                             JOIN `products` as i ON o.product_id = i.product_id
                             WHERE o.customer_id='{$_SESSION['customer_id']}' AND o.order_phase='6'";
                             

$confirmed_results = mysqli_query($conn, $sql_get_confirmed_orders);
if (!$confirmed_results) {
    die("Error fetching confirmed orders: " . mysqli_error($conn));
}
?>
<!DOCTYPE html>
<html lang="en">
...
<body>
    ...
    <div class="container">
        <div class="confirmed-orders">
            ...
            <?php if (mysqli_num_rows($confirmed_results) > 0) { ?>
                <table>
                    <tr>
                        <th>Product Name</th>
                        <th>Product Image</th>
                    ...
                    <?php while ($row = mysqli_fetch_assoc($confirmed_results)) { ?>
                        <tr>
                            <td><?php echo $row['product_name']; ?></td>
                            <td>
                                <img src="../uploads/<?php echo $row['product_image']; ?>" alt="<?php echo $row['product_name']; ?>" style="width:100px;height:auto;">
                            </td>
            ...
            <?php } else { ?>
                <p>You have no to shipped orders.</p>
            <?php } ?>
        </div>
    </div>
    </div>
